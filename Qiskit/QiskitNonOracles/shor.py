...
def modular_exponentiation(circuit, input, output, a, b):
    ...
    Parameters:
        input (QuantumRegister): The register representing X, the exponent of the power term.
            This can be in any superposition you want, so you can calculate the value for
            multiple inputs simultaneously.
        output (QuantumRegister): The register representing O, which will contain the
            solution to the function.
        a (int): The base of the power term
        b (int): The modulus
    """

    circuit.x(output[len(output) - 1])  # Output = |0...01>
	...
	# (Note: since Qiskit doesn't come with one of these functions, I ported Q#'s code over to Qiskit so I
