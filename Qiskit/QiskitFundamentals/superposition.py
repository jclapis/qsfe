...
class SuperpositionTests(unittest.TestCase):
    """
    This class contains some basic tests to show how Qiskit deals with qubits in superposition.
    ...
    def run_test(self, circuit, description, iterations, target_probabilities, margin):
        """
        ...
        # Get the |0〉 counts for each individual qubit from the total result measurement
        zero_counts = [0] * number_of_qubits
        for(state, count) in counts.items():
            for i in range(number_of_qubits):
                # If the i-th qubit was |0> for this state, add the state count
                # to that qubit.
                if(state[i] == '0'):
                    # Qiskit treats qubit registers as little-endian, so we have to reverse the order
                    # to put the measurement in the right spot for comparison
                    zero_counts[number_of_qubits - i - 1] += count
    ...
    def test_identity(self):
        ...
        are initialized to |0〉 in Qiskit.
        """
        
        # Construct the circuit
        target_probabilities = [1]
        qubits = QuantumRegister(len(target_probabilities))
        ...
        # Run the test
        self.run_test(circuit, "Identity", 10000, target_probabilities, 0)
    ...
    def test_invert(self):
        ...
        # Run the test
        self.run_test(circuit, "Invert", 10000, target_probabilities, 0)
    ...
    def test_hadamard(self):
        ...
        # Run the test
        self.run_test(circuit, "Hadamard", 10000, target_probabilities, 0.02)


    def test_arbitrary_rotation(self):
        """
        This tests arbitrary rotations around the Y axis (so the X-Z plane) to make sure Qiskit
        ...
        for i in range(2, 7):
            ...
            for j in range(0, i + 1):
                ...
                circuit.ry(angle, qubits[j])

            # Run the test
            self.run_test(circuit, f"Rotation with steps of 1/{i} ({step_string}%)", 2000, target_probabilities, 0.05)
