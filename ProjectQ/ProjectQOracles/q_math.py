...
def inplace_xor(classic_bits, qubits):
    """
    ...
    Parameters:
        ...
        qubits (Qureg): The qubit array to use in the XOR
    """
...
def left_shift(input, output, amount):
    """
    Left-shifts the input qubit array by the specified number of bits.

    Parameters:
        input (Qureg): The register that contains the qubits to shift.
            This can be in any arbitrary state.
        output (Qureg): The register that will hold the shifted qubits.
    ...
    for input_index in range(amount, len(input)):
        ...
        CNOT | (input[input_index], output[output_index])


def right_shift(input, output, amount):
    """
