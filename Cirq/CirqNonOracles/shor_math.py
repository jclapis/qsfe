...
def qft_reverse_register(circuit, register):
    ...
    Parameters:
        ...
        register (list[Qid]): The register to run QFT on after inversion
...
def controlled_apply_phase_operation_on_register(circuit, control, operation, target, operation_args):
    ...
    Parameters:
        circuit (Circuit): The circuit being constructed
        control (Qid): The control qubit
        operation (function): The operation to run
        target (list[Qid]): The register to run the operation on
...
def copy_most_significant_bit(circuit, register, target):
    ...
    circuit.append(cirq.CNOT(most_significant_qubit, target))
...
def controlled_modular_increment_phase(circuit, control_list, increment, modulus, target):
    ...
    less_than_modulus_flag = cirq.NamedQubit(name="less_than_modulus_flag")
    ...
    circuit.append(cirq.X(less_than_modulus_flag))
    apply_register_operation_on_phase(circuit, copy_most_significant_bit, target, less_than_modulus_flag)
    controlled_integer_increment_phase(circuit, control_list, increment, target)
...
def controlled_modular_add_product(circuit, control, constant, modulus, multiplier, summand):
    ...
    Parameters:
        ...
        multiplier (list[Qid]): A quantum register representing an unsigned integer whose value is to
            be added to each basis state label of "summand".
        summand (list[Qid]): A quantum register representing an unsigned integer to use as the target
            for this operation.
    ...
    extra_zero_bit = cirq.NamedQubit(name="extra_zero_bit")
...
def controlled_modular_multiply(circuit, control, constant, modulus, multiplier):
    ...
    controlled_modular_add_product(circuit, control, constant, modulus, multiplier, summand)
    ...
    controlled_adjoint_modular_add_product(circuit, control, inverse_mod_value, modulus, multiplier, summand)