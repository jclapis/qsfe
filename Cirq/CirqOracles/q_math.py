...
def inplace_xor(circuit, classic_bits, qubits):
    ...
    Parameters:
        ...
        qubits (list[Qid]): The qubit array to use in the XOR
    """
...
def left_shift(circuit, input, output, amount):
    ...
    Parameters:
        circuit (Circuit): The circuit being constructed
        input (list[Qid]): The register that contains the qubits to shift.
            This can be in any arbitrary state.
        output (list[Qid]): The register that will hold the shifted qubits.
    ...
    for input_index in range(amount, len(input)):
        ...
        circuit.append(cirq.CNOT(input[input_index], output[output_index]))
