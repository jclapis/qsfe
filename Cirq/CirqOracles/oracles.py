...
def always_zero(circuit, qubits, target):
    ...
    Parameters:
        ...
        target (list[Qid]): The target qubit to flip if the input
...
def check_if_all_ones(circuit, qubits, target):
    ...
    circuit.append(cirq.X(target).controlled_by(*qubits))
...
def check_if_all_zeros(circuit, qubits, target):
    ...
    circuit.append([
        ...
        cirq.X(target).controlled_by(*qubits),
        cirq.X.on_each(*qubits)
    ])
...
def check_for_odd_number_of_ones(circuit, qubits, target):
    ...
    for qubit in qubits:
        circuit.append(cirq.CNOT(qubit, target))
...
def check_if_qubit_is_one(circuit, qubits, target, index):
    ...
    circuit.append(cirq.CNOT(qubits[index], target))
...
def check_xor_pad(circuit, pad_key, target, extra_args):
    ...
    Parameters:
        circuit (Circuit): The circuit being constructed
        pad_key (list[Qid]): The qubit array containing the candidate
            pad key that you want to check for correctness.
        target (list[Qid]): The target qubit in the oracle. If the pad
