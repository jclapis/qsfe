...
def swap_register(program, qubits):
    """
    Swaps all of the qubits in a register, effectively reversing it.

    Parameters:
        program (Program): The program being constructed
        qubits (list[QubitPlaceholder]): The register to reverse
    """

    for i in range(0, len(qubits) // 2):
        program += SWAP(qubits[i], qubits[len(qubits) - 1 - i])
...
def qft(qubits):
    ...
    Parameters:
        qubits (list[QubitPlaceholder]): The register to apply the QFT to

    Returns:
        A program containing the QFT gates.
