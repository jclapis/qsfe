...
def always_zero(program, qubits, target):
    """
    ...
    Parameters:
        ...
        target (QubitPlaceholder): The target qubit to flip if the input
...
def always_one(program, qubits, target):
    """
...
def check_if_all_ones(program, qubits, target):
    """
    ...
    gate = X(target)
    for qubit in qubits:
        gate = gate.controlled(qubit)
    program += gate


def check_if_all_zeros(program, qubits, target):
    """
    This is a quantum oracle that will flip the target qubit if
    ...
    check_if_all_ones(program, qubits, target)
    
    for qubit in qubits:
        program += X(qubit)
    

def check_if_register_matches_bit_string(program, qubits, target, bit_string):
    """
    This is a quantum oracle that will flip the target qubit if
...
def check_for_odd_number_of_ones(program, qubits, target):
    """
    ...
    Parameters:
        program (Program): The program being constructed
        qubits (list[QubitPlaceholder]): The register to run the oracle on
        target (list[QubitPlaceholder]): The target qubit to flip if the input
            satisfies the oracle
    """
...
def check_if_qubit_is_one(program, qubits, target, index):
    """
    ...
    program += CNOT(qubits[index], target)


def check_xor_pad(program, pad_key, target, extra_args):
    """
    ...
    Parameters:
        program (Program): The program being constructed
        pad_key (list[QubitPlaceholder]): The qubit array containing the candidate
            pad key that you want to check for correctness.
        target (QubitPlaceholder): The target qubit in the oracle. If the pad
    ...
    # the desired state
    q_math.inplace_xor(program, encoded_message, pad_key)
    check_if_register_matches_bit_string(program, pad_key, target, desired_result)
    q_math.inplace_xor(program, encoded_message, pad_key)
